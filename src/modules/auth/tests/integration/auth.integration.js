...
describe('Auth API Integration Test', () => {
  ...
  describe('Signin User POST: /api/v1/auth/signin', () => {
    ...
    it('should return error when auth user is deleted', async () => {
      // Delete the active user
      await User.update({ email: dbUsers.activeUser.email }, { $set: { deleted: true } });
      const signinUser = {
        email: dbUsers.activeUser.email,
        password,
      };
      return request(server)
        ...
        .then((res) => {
          expect(res.body).to.have.a.property('errors');
          expect(res.body.name).is.equal('Error');
          expect(res.body.message).is.equal('Cannot login, please contact admin for more details');
        });
    });
